    ...
    public class Harmony_Explosion
    {
            ...
            Func<T, IntVec3, int> getDamage) where T : Explosion
        {
            ...
            cellsToAffect
                ...
                .Where(cell => TryBlock(explosion.Map, origin, explosion.damType, getDamage(explosion, cell),
        ...
        {
            [HarmonyPrefix]
            private static void HandleOuterEdgesFirst(Explosion __instance, List<IntVec3> ___cellsToAffect,
                int ___startTick)
            {
        ...
        {
            [HarmonyPrefix]
            private static bool CheckCellShielded(Explosion __instance, IntVec3 c)
            {
                return !TryBlock(__instance.Map, __instance.TrueCenter().Yto0(), __instance.damType,
                    GetDamage(__instance, c), PositionUtility.ToVector3(c).Yto0());
            }
        }
    }
}