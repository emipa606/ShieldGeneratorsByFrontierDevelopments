...
namespace FrontierDevelopments.Shields.Harmony
{
    public class Harmony_CellFinderLoose
    {
        ...
        {
            if (Mod.Settings.EnableAIFleeToShields)
            {
                return num;
            }
        ...
        [HarmonyPatch]
        [UsedImplicitly]
        private static class Patch_GetFleeDestToolUser
        {
            [HarmonyTargetMethod]
            ...
            [HarmonyTranspiler]
            private static IEnumerable<CodeInstruction> AddShieldCheck(IEnumerable<CodeInstruction> instructions,
                ILGenerator il)
            {
            ...
            private class Transpiler_AddShieldCheck
            {
                ...
                public IEnumerable<CodeInstruction> Apply(IReadOnlyCollection<CodeInstruction> instructions)
                {
                    ...
                    Log.Warning(
                        "Failed patching CellFinderLoose.GetFleeDestToolUser, pawns won't flee to shielded areas");
                    return instructions;
                }
            }
        }
