...
namespace FrontierDevelopments.Shields.Harmony
{
    public class Harmony_Skyfaller
    {
        ...
        private static readonly FieldInfo autosaverTicksSinceLast =
            AccessTools.Field(typeof(Autosaver), "ticksSinceSave");
        ...
        {
            if (!Mod.Settings.AutosaveOnSkyfallers)
            {
                return;
            }
            ...
            if (AutosaveIfNeeded())
            {
                Log.Message("Frontier Development Shields :: " + skyfaller.Label +
                            " can fall within a shield, autosaving");
            }
        }
        ...
        private static class Patch_Tick
        {
            ...
            {
                if (!Mod.Settings.BlockSkyfallers)
                {
                    return true;
                }

                if (__instance.Map == null || __instance.def.skyfaller.reversed ||
        ...
        [UsedImplicitly]
        private static class Patch_SpawnSetup
        {
            [HarmonyPostfix]
            private static void HandleAutosaveIfCanImpactShields(Skyfaller __instance, Map map)
            {
