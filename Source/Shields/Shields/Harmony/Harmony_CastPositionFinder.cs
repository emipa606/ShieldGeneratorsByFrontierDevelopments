...
namespace FrontierDevelopments.Shields.Harmony
{
    public class Harmony_CastPositionFinder
    {
        private static float CalculateShieldPreference(float score, Map map, IntVec3 start, IntVec3 end)
        {
            ...
            var targetProtected = shieldManager.Query(map).IsActive().Intersects(startAdjusted, endAdjusted).Get()
                .ToList();
            var shooterProtected = shieldManager.Query(map).IsActive().Intersects(endAdjusted, startAdjusted).Get()
                .ToList();
