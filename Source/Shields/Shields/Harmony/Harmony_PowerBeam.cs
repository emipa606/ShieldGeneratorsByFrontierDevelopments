...
{
    internal class Harmony_PowerBeam : Harmony_OrbitalStrike
    {
        ...
        private static class Patch_StartRandomFire
        {
            ...
            private class Transpiler_ProtectShieldedThings
            {
                    ...
                    foreach (var instruction in instructions)
                    {
                        if (Success == false && instruction.opcode == OpCodes.Stloc_0)
                        {
                            ...
                            yield return new CodeInstruction(OpCodes.Call,
                                AccessTools.Property(typeof(PowerBeam), nameof(PowerBeam.Map)).GetGetMethod());
                    ...
                    ILGenerator il)
                {
                    ...
                    Log.Warning(
                        "Failed patching PowerBeam.StartRandomFireAndDoFlameDamage, protecting pawns and stopping in range is disabled");
                    return instructions;
                }
            }
        }
